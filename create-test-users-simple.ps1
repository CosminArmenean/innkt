...
foreach ($user in $testUsers) {
    ...
    try {
        ...
        Write-Host "User $($user.email) might already exist, trying login..." -ForegroundColor Yellow
        ...
        try {
            ...
            Write-Host "Failed to create/login user $($user.email)" -ForegroundColor Red
        }
    }
}
